      ...
      <div>
        ...
        <div className="space-y-2">
          {recipe.other.map((ingredient, index) => {
            console.log(`[RecipeForm] Rendering ingredient ${index}:`, ingredient.name, `Unit: '${ingredient.unit}' (Type: ${typeof ingredient.unit})`);
            ...
            return (
              <div key={index} className="flex flex-wrap items-center space-x-2 p-2 border border-stone-200 rounded">
                ...
                <select 
                  ...
                  className="w-24 px-2 py-1 border border-stone-300 rounded-md shadow-sm sm:text-sm text-stone-900 bg-white"
                  disabled={isSaving || isDeleting}
                >
